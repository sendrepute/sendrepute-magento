...
was not created for those offline fixtures; see the separate installed-host
experiment below. Typed fixtures alone are not broader version certification.

## Disposable installed-host security suite

`sh tests/run-installed.sh` builds a disposable Magento install from the public
GitHub source and pinned lockfile, owns a local MariaDB socket, starts isolated
Elasticsearch 7 on loopback ports 34705/34706 and an HTTP admin on 34700. It
never reuses an existing work root. Run within a package shell containing PHP
8.3, Composer, php83Extensions.xsl and elasticsearch7:

```sh
nix-shell -p php83 php83Packages.composer php83Extensions.xsl elasticsearch7 \
  --run 'sh tests/run-installed.sh'
```

`tests/installed-result.json` is generated by the **passing automated fresh
install** (Magento setup 901/901), not by fixtures or bootstrap alone. It
records the exact source commit, source and lockfile hashes, versions, and
per-check coverage. `tests/run-focused.sh /tmp/sendrepute-magento-installed.XXXXXX/site`
repeats the native security checks on an already installed, owned disposable
site without downloading or reinstalling; run it in the same PHP 8.3 + xsl
environment as the installer, with `PHP_INI_SCAN_DIR` pointing to that runner's
PHP configuration if starting a fresh Nix shell. Both scripts reject unrelated
existing sites.

The native host checks cover guest admin redirect, login, authenticated
configuration rendering, missing-form-key CSRF rejection, stored script markup
escaping, and denial to an actual dashboard-only restricted admin account.
At Magento's final transport, a synthetic classifier drives both API-failure
policy and high-risk cancellation before native delivery; low-risk selected
messages reach the owned sendmail sink. Unselected and consent-off messages
also reach that sink without classification. Four separate PHP workers send
identical content simultaneously: with consent off there are four deliveries
and zero calls; with consent on there are four independent synthetic
classifications and four deliveries. This is **not** an idempotency guarantee:
the adapter has no nonce, automatic retry or cross-worker deduplication, so
those product mechanisms are not applicable. `SENDREPUTE_API_TOKEN` is unset,
the classifier is replaced only inside the disposable test process, and the
local mail sink never invokes sendmail or external SMTP. No external mail or
paid classification occurred. The suite does not certify third-party
transports, other patch releases, or a real remote classifier response.

The initial host PHP 8.4.10 was not used: Magento 2.4.7-p6's official root
composer.json requires PHP 8.1 through 8.3. Nix PHP 8.3.23 plus a separately
enabled xsl extension worked without overriding Composer platform requirements.
Elasticsearch 7.17.27 was provisioned through Nix without Docker.

Credential-free acquisition uses the public official
2.4.7-p6 GitHub source archive (HTTP 200), and its committed
composer.lock contains 133 dependency archives hosted at api.github.com.
This public-source approach completed Composer resolution and Magento setup.
`repo.magento.com/packages.json` warns of absent Magento
authentication keys (and its direct HEAD responds 401), so do not use
`composer create-project` against that private distribution or ask for keys.
The GitHub release-asset URL tested for `magento-ce-2.4.7-p6.tar.gz`
responded 404. No `--ignore-platform-reqs` workaround was used.
